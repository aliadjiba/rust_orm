...
use crate::{model::{Edge, Insert, Model, Page, SqlState}, pluralizer, repository::Repo};
...
impl<'a, M, R> Query<'a, M, R>
where
    M: Model + SurrealValue + Serialize,
    R: DeserializeOwned + SurrealValue,
{
    ...
    pub fn where_eq<V: Serialize + SurrealValue>(
        mut self,
        field: &str,
        value: V,
    ) -> Self {
    ...
    pub fn by_id<V: Serialize + SurrealValue >(
        mut self,
        value: V,
    ) -> Self {
        let key = self.state.bind(value);
        ...
        V: Serialize + SurrealValue
    {
        ...
        if !parts.is_empty() {
            self.state.conditions.push(format!("({})", parts.join(" OR ")));
        }
        self
    }
    ...
    pub fn where_in<V: Serialize + Send + Sync + SurrealValue  + 'static,>(
        ...
        let key = self.state.bind(values);
        self.state.conditions.push(format!("{field} IN ${key}"));
        self
    }
   ...
   pub async fn all(self) -> Result<Vec<R>, ErrorIO> {
    ...
    if let Some(with) = self.with {
        for w in with {
            if pluralizer::is_plural(&w){
                let s = pluralizer::singularize(&w);
                select.push_str(
                    // &format!(" , {s}.* AS {w} ")
                    &format!(" , (SELECT * FROM {} WHERE {} = $parent.id) AS {w} ", quote_table(&s), M::table_name())
                );
            }else {
                select.push_str(
                    &format!(" , {w}.* ")
    ...
    let mut sql =
        format!("SELECT {select} FROM {} ", quote_table(self.table));
